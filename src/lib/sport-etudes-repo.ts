...
/* ── Paiement en 2 versements — miroir de la section équivalente de
 *  season-admin-repo.ts, mais avec sa propre table (FK vers
 *  sport_etudes_registrations, pas registrations). Voir
 *  src/lib/payment-plan.ts::getSportEtudesInstallmentPlan pour le calcul. */

export interface CreateSportEtudesPaymentPlanInstallmentInput {
  sequenceNo: number;
  amountCents: number;
  dueDate: string; // 'YYYY-MM-DD'
}

export interface CreateSportEtudesPaymentPlanInput {
  registrationId: string;
  totalAmountCents: number;
  installments: CreateSportEtudesPaymentPlanInstallmentInput[];
}

export async function createPaymentPlan(input: CreateSportEtudesPaymentPlanInput): Promise<string> {
  const supabase = db();
  const { data, error } = await supabase
    .from("sport_etudes_payment_plans")
    .insert({
  ...
  const { error: instError } = await supabase.from("sport_etudes_payment_plan_installments").insert(rows);
...
export async function deletePaymentPlan(planId: string): Promise<void> {
  const supabase = db();
  await supabase.from("sport_etudes_payment_plan_installments").delete().eq("plan_id", planId);
  await supabase.from("sport_etudes_payment_plans").delete().eq("id", planId);
}

/** Active le plan une fois le 1er versement confirmé par le webhook Stripe :
 *  enregistre la carte pour le prélèvement automatique du 2e versement, et
 *  marque l'installment #1 comme payé. */
export async function activatePaymentPlan(
  ...
  const { error: instError } = await supabase
    .from("sport_etudes_payment_plan_installments")
    .update({
      status: "paid",
...
const MAX_SPORT_ETUDES_INSTALLMENT_ATTEMPTS = 5;

export interface SportEtudesDueInstallment {
  id: string;
  plan_id: string;
  sequence_no: number;
  amount_cents: number;
...
/** Versements Sport-Études dus aujourd'hui (ou en retard), hors 1er
 *  versement (déjà payé via Stripe Checkout à l'inscription) et hors ceux
 *  ayant atteint le nombre maximal de tentatives — utilisé par
 *  /api/cron/charge-installments. */
export async function getDueInstallments(): Promise<SportEtudesDueInstallment[]> {
  const supabase = db();
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      sport_etudes_payment_plans!inner (
        stripe_customer_id, stripe_payment_method_id, installment_count,
        sport_etudes_registrations!inner ( parent_first_name, parent_last_name, parent_email )
      )
    `)
    .in("status", ["pending", "failed"])
    .gt("sequence_no", 1)
    .lte("due_date", today)
    .lt("attempt_count", MAX_SPORT_ETUDES_INSTALLMENT_ATTEMPTS);
  if (error) throw new Error(error.message);
  ...
  return (data ?? []).map((row: any) => ({
    ...
    stripe_customer_id: row.sport_etudes_payment_plans.stripe_customer_id,
    stripe_payment_method_id: row.sport_etudes_payment_plans.stripe_payment_method_id,
    installment_count: row.sport_etudes_payment_plans.installment_count,
    parent_name: `${row.sport_etudes_payment_plans.sport_etudes_registrations.parent_first_name} ${row.sport_etudes_payment_plans.sport_etudes_registrations.parent_last_name}`.trim(),
    parent_email: row.sport_etudes_payment_plans.sport_etudes_registrations.parent_email
  }));
}
...
export async function markInstallmentFailed(
  ...
  const isFinal = attemptCount >= MAX_SPORT_ETUDES_INSTALLMENT_ATTEMPTS;
